...
| **05** | `05-vietnamese-ime.sh` | Cài đặt Fcitx5 kèm bộ gõ Lotus (không gạch chân kiểu macOS/iOS), cấu hình phím tắt chuyển ngôn ngữ `Alt + Left Shift`. |
| **06** | `06-dev-environment.sh` | Cài đặt môi trường phát triển: PHP 8.5, Composer, Node.js 24 LTS, npm, Symfony CLI. |
| **07** | `07-keybinds.sh` | Phím tắt chuyển workspace (`Super + 1..9`), chuyển màn hình (`Super + Alt + 1..3`), chụp màn hình vùng chọn không cần Enter (`Super + Shift + S`) kèm popup chỉnh sửa với Satty. |
...
| **09** | `09-dms-plugins.sh` | Cài đặt plugin `dms-antigravity-cli`, cấu hình hiển thị trên taskbar bên phải, patch tự động lấy email tài khoản đang đăng nhập để hiển thị trên widget. |
...
Hệ thống sẽ tự động thực thi lần lượt từ module 01 đến 10:
```bash
cd ~/setup-scripts
./setup.sh
# hoặc: ./setup.sh all
